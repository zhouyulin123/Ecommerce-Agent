...
class CopywritingNode:
    ...
    def run(self, entities: Dict[str, Any], insight: Dict[str, Any], memory: ConversationMemory, feedback: Dict[str, Any]) -> Dict[str, Any]:
        llm_result = self._normalize_copy_result(self._llm_generate(entities, insight, memory, feedback))
        if llm_result:
            return llm_result
        return self._fallback_generate(entities, insight, memory, feedback)

    def _llm_generate(self, entities: Dict[str, Any], insight: Dict[str, Any], memory: ConversationMemory, feedback: Dict[str, Any]) -> Dict[str, Any]:
        payload = {"entities": entities, "insight": insight, "preference_memory": memory.preference_memory, "feedback": feedback, "previous_copy": memory.last_artifacts.get("ad_copy")}
    ...
    def _normalize_copy_result(result: Dict[str, Any]) -> Dict[str, Any]:
        ...
        subtitle = result.get("subtitle") or result.get("小标题") or result.get("副标题") or result.get("文案") or ""
        cta = result.get("cta") or result.get("行动号召") or result.get("按钮文案") or "立即查看"
        normalized = {"title": str(title).strip(), "subtitle": str(subtitle).strip(), "cta": str(cta).strip()}
        if not normalized["title"] and not normalized["subtitle"]:
            return {}
        return normalized

    @staticmethod
    def _fallback_generate(entities: Dict[str, Any], insight: Dict[str, Any], memory: ConversationMemory, feedback: Dict[str, Any]) -> Dict[str, Any]:
        ...
        return {"title": title[:18], "subtitle": subtitle[:35], "cta": "立即查看" if not avoid_hard_sell else "了解详情"}
