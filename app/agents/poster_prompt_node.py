...
class PosterPromptNode:
    ...
    def _llm_generate(
        ...
        """调用 LLM 生成 `poster_prompt/visual_elements/color_palette/style_keywords`。"""
        payload = {
            "entities": entities,
            "ad_copy": ad_copy,
            "preference_memory": memory.preference_memory,
            "feedback": feedback,
            "previous_poster": memory.last_artifacts.get("poster_spec"),
        }
    ...
    def _fallback_generate(
        ...
        prompt += (
            f"，整体风格{', '.join(style_keywords[-3:])}"
            f"，配色{palette}，构图干净，适合电商营销图生成。"
        )
